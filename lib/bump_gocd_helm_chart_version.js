...
chartYamlContent = chartYamlContent.replaceAll(GOCD_CURRENT_VERSION, GOCD_VERSION_TO_RELEASE);
...
chartYamlContent = chartYamlContent.replaceAll(currentAppVersion, newAppVersion);
fs.writeFileSync(chartYamlFilePath, chartYamlContent);
console.log(`Done updating chart version..`);
...
const changelogFilePath = `${CLONE_TO_PATH}/gocd/CHANGELOG.md`;
...
console.log(childProcess.execSync(`git add gocd/CHANGELOG.md`, options).toString());
